...
import { Check, ChevronDown, Home, LogOut, PanelLeftClose, Plus, Search, Settings, Sparkles, SquarePen, UserPlus, X } from "lucide-react";
...
const DEFAULT_PRIMARY_ACTION: SidebarNavItem = { key: "chats", label: "New chat", icon: <SquarePen size={18} /> };

const DEFAULT_WORKSPACE_ACTIONS: SidebarWorkspaceAction[] = [
  ...
  { label: "Sign out", icon: <LogOut size={16} />, separated: true },
];
...
export default function SidebarNav({
  ...
  workspaceActions = DEFAULT_WORKSPACE_ACTIONS,
}: SidebarNavProps) {
        ...
        {historyLabel ? <div className="mt-3 min-h-0 flex-1 overflow-y-auto">
          <div className="sidebar-copy relative mx-2 mb-1 h-8">
            ...
            >
              ...
              <button
                type="button"
                  aria-label={`Close ${searchPlaceholder.toLowerCase()}`}
                onClick={() => {
                  setSearchOpen(false);
                  setQuery("");
                }}
