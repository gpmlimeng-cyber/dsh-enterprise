 ...
 * [OUTPUT]: 提供六个导航页面、隐藏用户中心的静态元数据、路径判断和多角色页面并集。
 * [POS]: app 的唯一控制台路由与前端页面可见性真源，Server ent:* 权限仍独立裁决 API。
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */

import { Activity, BookOpen, Boxes, CircleUserRound, Puzzle, ShieldCheck, Users, type LucideIcon } from 'lucide-react';
import type { AuthBuiltInRole } from '@/api/generated/types.gen';

type ProductRouteDefinition = {
  to: '/' | '/access' | '/plugins' | '/presets' | '/members' | '/activity';
  label: string;
  icon: LucideIcon;
