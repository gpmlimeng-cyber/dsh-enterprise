...
audit.yaml: 39-action 枚举、封闭 metadata 与 cursor；模型传输失败可携带已实测结算，明确拒绝可 RELEASED，恢复可由 usage 快照 SETTLED。
cloud-project.yaml: 云端项目 id/slug/role、create 请求、cloneUrl 投影与成员 schema 分片；不投影磁盘路径或 Git 内部状态。
