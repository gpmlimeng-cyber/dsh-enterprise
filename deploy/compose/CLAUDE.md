...
compose.yml: 默认拉取 GHCR `next` Server/Console，以带默认值的环境变量和可覆盖 `admin/owndsh` 初始化管理员；PostgreSQL 仅建数据库/账号，Server Flyway 包含 V0 并接管全部库内初始化；签名默认关闭且不提供私钥，源码与离线 release 共用拓扑，数据组件 digest 固定且仅 Console 发布 HTTP 8080；持久卷只保存 PostgreSQL、Redis、插件制品与云端工作空间 bare 仓库，storage-init 初始化插件与云端仓库卷权限。
