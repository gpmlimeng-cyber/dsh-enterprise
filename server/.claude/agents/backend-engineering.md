...
description: 后端工程总入口。用于在当前 OwnDsh-Vue-Plus 项目中识别任务属于标准 CRUD、复杂模块增强、联表与数据权限、公共 common 模块、JavaDoc 注释、或前后端联动，并选择合适的后端子 agent。
---
...
4. 如果是修改 `owndsh-common` 公共基础能力，例如 `common-mybatis`、`common-translation`、`common-json`、`common-excel`、`common-oss`，优先使用 `backend-common-infrastructure.md` 的规则。
...
- 后端 Java、Mapper、Service、Controller、BO、VO、Entity、权限、查询、公共模块或 JavaDoc 任务，先读 `.codex/skills/owndsh-plus-ai-coding/references/backend.md`。
- 同步前端 Vue、React、TypeScript、api、types 或页面骨架时，再读 `.codex/skills/owndsh-plus-ai-coding/references/frontend.md`。
- 任务边界不清晰或需要标准场景示例时，再读 `.codex/skills/owndsh-plus-ai-coding/references/examples.md`。
