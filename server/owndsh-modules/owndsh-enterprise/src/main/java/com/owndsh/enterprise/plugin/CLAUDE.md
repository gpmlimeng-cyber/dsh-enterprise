...
EnterprisePluginConfiguration.java: 插件服务端 composition root，装配归档检查、与 inspector 同上限的 zip/tgz 归一化器、默认关闭的 Ed25519 签名、CAS 制品库、JDBC ports 和事务服务。
EnterprisePluginProperties.java: artifact root、默认 false 的 signingEnabled 与可选环境 PKCS#8 私钥、三项归档上限的部署配置边界。
artifact/: 不信任 tgz/zip 的容器归一化与流式校验、RFC 8785 签名声明和内容寻址文件存储；局部地图见 artifact/CLAUDE.md。
