...
`READY`/`REFRESHING`. A fresh install asks only for the DSH Enterprise Server origin,
...
Server change restore the gate. Explicit uninstall removes DSH Enterprise and its
...
the embedded DSH Enterprise whale at the native Settings icon size, matches its row
...
When the account becomes blocked, the DSH Enterprise Settings section uses the official
