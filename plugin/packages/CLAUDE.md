...
session-sync/: 企业 Session 同步客户端 `@dshent/session-sync`；P2b 上传链路（dirty/防抖/切批/T16 batches/游标/终态）经结构端口注入，不导入 dsh-session 真包，未接入 bundle（V1 门禁仍禁 `enterpriseSessionSync`）。
