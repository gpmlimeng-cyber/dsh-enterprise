...
src/local-api.ts: exact/prefix 同源路由暴露企业目录、严格 package/version 安装与 package 卸载动作，以及 Server/账号/Session/云端项目 JSON 与显式刷新；`actionErrorStatus` 是各模块错误码到 HTTP 状态的注册表，新增模块必须登记否则统一退化为 503；执行端口由 bundle 反向注入以避免依赖环。
